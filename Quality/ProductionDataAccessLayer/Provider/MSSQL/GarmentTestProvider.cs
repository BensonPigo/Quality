...
";
                        ...
                        if (dt_Location.Select("Location = 'T'").Any() || dt_Location.Select("Location = 'B'").Any() || dt_Location.Select("Location = 'I'").Any() || dt_Location.Select("Location = 'O'").Any())
                        {
                            ...
                            if (dt_Location.Select("Location = 'I'").Any())
                            {
                                sqlcmd_Spirality += $@"INSERT INTO[dbo].[Garment_Detail_Spirality]([ID],[No],[Location])VALUES('{master.ID}','{item.No}','I');";
                            }

                            if (dt_Location.Select("Location = 'O'").Any())
                            {
                                sqlcmd_Spirality += $@"INSERT INTO[dbo].[Garment_Detail_Spirality]([ID],[No],[Location])VALUES('{master.ID}','{item.No}','O');";
                            }
...
,1,1,'')";
                    ...
                    {
                        fGPTs = GetDefaultFGPT(isRugbyFootBall, "S");
                    }
                    else if (containsT)
                    {
                        fGPTs = GetDefaultFGPT(isRugbyFootBall, "T");
                    }
                    else if (containsB)
                    {
                        fGPTs = GetDefaultFGPT(isRugbyFootBall, "B");
                    }
                    else if (containsI || containsO)
                    {
                        fGPTs = new List<GarmentTest_Detail_FGPT>();
                        if (containsI)
                        {
                            fGPTs.AddRange(GetDefaultFGPT(isRugbyFootBall, "I"));
                        }

                        if (containsI && containsO)
                        {
                            var itemToRemove = fGPTs.SingleOrDefault(x => x.Type == "odour: Garment" && x.TestName == "PHX-AP0451");
                            if (itemToRemove != null)
                                fGPTs.Remove(itemToRemove);
                        }

                        if (containsO)
                        {
                            fGPTs.AddRange(GetDefaultFGPT(isRugbyFootBall, "O"));
                        }
                    }
                    else
                    {
                        fGPTs = GetDefaultFGPT(isRugbyFootBall, string.Empty);
                    }
...
";
                ...
                List<GarmentTest_Detail_ViewModel> needUpdateDetailList = PublicClass.CompareListValue<GarmentTest_Detail_ViewModel>(
                    detail, oldDetailData, "ID,No", "OrderID,SizeCode,MtlTypeID,SubmitDate,inspector,NonSeamBreakageTest,Remark,Approver,Receiver,ReceiveDate");
                ...
                foreach (GarmentTest_Detail_ViewModel detailItem in needUpdateDetailList)
                {
                    ...
                    switch (detailItem.StateType)
                    {
                        case CompareStateType.Add:
                            ...
                            objParameterDetail.Add($"@Receiver", string.IsNullOrEmpty(detailItem.Receiver) ? string.Empty : detailItem.Receiver);
                            objParameterDetail.Add($"@ReceiveDate", detailItem.ReceiveDate);
                            ...
                            objParameterDetail.Add($"@SubmitDate", detailItem.SubmitDate);
...
";
        ...
        public static List<GarmentTest_Detail_FGPT> GetDefaultFGPT(bool isRugbyFootBall, string location)
        {
            ...
            List<GarmentTest_Detail_FGPT> innerOnly = new List<GarmentTest_Detail_FGPT>()
            {
                new GarmentTest_Detail_FGPT() { Seq = 3, Location = "Inner", TestDetail = "mm", TestUnit = "mm", TestName = "PHX-AP0413", Type = "seam slippage: Garment - weft - upper bodywear 150N", Criteria = 4 },
                new GarmentTest_Detail_FGPT() { Seq = 6, Location = "Inner", TestDetail = "mm", TestUnit = "mm", TestName = "PHX-AP0413", Type = "seam slippage: Garment - warp - upper bodywear 150N", Criteria = 4 },
                new GarmentTest_Detail_FGPT() { Seq = 9, Location = "Inner", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No fabric breakage: Garment - weft - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 12, Location = "Inner", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No fabric breakage: Garment - warp - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 15, Location = "Inner", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No seam breakage: Garment - weft - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 18, Location = "Inner", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No seam breakage: Garment - warp - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 1, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Side seam - Method B  ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 2, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Armhole seam - Method B ≥180N)", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 3, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Under arm seam or sleeve seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 4, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Shoulder seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 5, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Waistband seam  - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 6, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Hood seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 7, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 8, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 9, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upperr body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 19, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper  body wear/ full body wear (Side seam - Method A ≥70N)", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 20, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Armhole seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 21, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upperr body wear/ full body wear (Under arm seam or sleeve seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 22, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Shoulder seam - Method A ≥70N)", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 23, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Neck seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 24, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Waistband seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 25, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Hood seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 26, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 27, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 28, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 29, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Side seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 30, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Armhole seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 31, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Under arm seam or sleeve seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 32, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Shoulder seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 33, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Waistband seam  - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 34, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Hood seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 35, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 36, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 37, Location = "Inner", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upperr body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
            };

            List<GarmentTest_Detail_FGPT> outerOnly = new List<GarmentTest_Detail_FGPT>()
            {
                new GarmentTest_Detail_FGPT() { Seq = 3, Location = "Outer", TestDetail = "mm", TestUnit = "mm", TestName = "PHX-AP0413", Type = "seam slippage: Garment - weft - upper bodywear 150N", Criteria = 4 },
                new GarmentTest_Detail_FGPT() { Seq = 6, Location = "Outer", TestDetail = "mm", TestUnit = "mm", TestName = "PHX-AP0413", Type = "seam slippage: Garment - warp - upper bodywear 150N", Criteria = 4 },
                new GarmentTest_Detail_FGPT() { Seq = 9, Location = "Outer", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No fabric breakage: Garment - weft - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 12, Location = "Outer", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No fabric breakage: Garment - warp - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 15, Location = "Outer", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No seam breakage: Garment - weft - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 18, Location = "Outer", TestDetail = "pass/fail", TestUnit = "N", TestName = "PHX-AP0413", Type = "No seam breakage: Garment - warp - upper bodywear 150N", Criteria = 150 },
                new GarmentTest_Detail_FGPT() { Seq = 1, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Side seam - Method B  ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 2, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Armhole seam - Method B ≥180N)", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 3, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Under arm seam or sleeve seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 4, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Shoulder seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 5, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Waistband seam  - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 6, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear (Hood seam - Method B ≥180N )", Criteria = 180 },
                new GarmentTest_Detail_FGPT() { Seq = 7, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 8, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 9, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage after wash (only for welded/bonded seams): Garment - width direction - Upperr body wear/ full body wear ({0}- Method B ≥180N ) Other Joining seam  selection", Criteria = 180, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 19, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper  body wear/ full body wear (Side seam - Method A ≥70N)", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 20, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Armhole seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 21, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upperr body wear/ full body wear (Under arm seam or sleeve seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 22, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Shoulder seam - Method A ≥70N)", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 23, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Neck seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 24, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Waistband seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 25, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear (Hood seam - Method A ≥70N )", Criteria = 70 },
                new GarmentTest_Detail_FGPT() { Seq = 26, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 27, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 28, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - length direction - Upper body wear/ full body wear ({0}- Method A ≥70N ) Other Joining seam  selection", Criteria = 70, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 29, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Side seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 30, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Armhole seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 31, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Under arm seam or sleeve seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 32, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Shoulder seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 33, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Waistband seam  - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 34, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear (Hood seam - Method B ≥140N )", Criteria = 140 },
                new GarmentTest_Detail_FGPT() { Seq = 35, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 36, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upper body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
                new GarmentTest_Detail_FGPT() { Seq = 37, Location = "Outer", TestDetail = "N", TestUnit = "N", TestName = "PHX-AP0450", Type = "seam breakage: Garment - width direction - Upperr body wear/ full body wear ({0}- Method B ≥140N ) Other Joining seam  selection", Criteria = 140, TypeSelection_VersionID = 1, TypeSelection_Seq = 1 },
            };
            ...
            switch (location)
            {
                ...
                case "S":
                    ...
                    break;
                case "I":
                    defaultFGPTList.AddRange(innerOnly);
                    break;
                case "O":
                    defaultFGPTList.AddRange(outerOnly);
                    break;
            }
