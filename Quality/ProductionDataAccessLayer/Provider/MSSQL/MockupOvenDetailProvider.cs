...
namespace ProductionDataAccessLayer.Provider.MSSQL
{
    ...
    {
        #region 底層連線
        public MockupOvenDetailProvider(string ConString) : base(ConString) { }
        public MockupOvenDetailProvider(SQLDataTransaction tra) : base(tra) { }
        #endregion

        #region CRUD Base
        public IList<MockupOven_Detail> Get(MockupOven_Detail Item)
        {
            ...
            SbSql.Append("FROM [MockupOven_Detail]" + Environment.NewLine);
            ...
            return ExecuteList<MockupOven_Detail>(CommandType.Text, SbSql.ToString(), objParameter);
        }

        public int Create(MockupOven_Detail Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
        ...
        public int Update(MockupOven_Detail Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
            SbSql.Append("UPDATE [MockupOven_Detail]" + Environment.NewLine);
...
FROM MockupOven_Detail md
inner join MockupOven m on m.ReportNo = md.ReportNo
Where 1=1
");
            if (!string.IsNullOrEmpty(Item.ReportNo))
            {
