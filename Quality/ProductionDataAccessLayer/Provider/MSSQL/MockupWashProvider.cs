...
namespace ProductionDataAccessLayer.Provider.MSSQL
{
    ...
    public class MockupWashProvider : SQLDAL, IMockupWashProvider
    {
        ...
        public int CreateMockupWash(MockupWash Item, string Mdivision, out string NewReportNo)
        {
...
");
        ...
        public void UpdateMockupWash(MockupWash_ViewModel Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
            SbSql.Append($@"
...
if not exists (select 1 from SciPMSFile_MockupWash where ReportNo = @ReportNo)
begin
    INSERT INTO SciPMSFile_MockupWash (ReportNo,TestBeforePicture,TestAfterPicture)
    VALUES (@ReportNo,@TestBeforePicture,@TestAfterPicture)
end
...
" + Environment.NewLine);
            ...
            objParameter.Add("@T2Supplier", DbType.String, HttpUtility.HtmlDecode(Item.T2Supplier) ?? string.Empty);
            ...
            objParameter.Add("@Approver", DbType.String, HttpUtility.HtmlDecode(Item.MockupWash_Approver) ?? string.Empty);
            if (Item.TestBeforePicture != null) { objParameter.Add("@TestBeforePicture", Item.TestBeforePicture); }
            else { objParameter.Add("@TestBeforePicture", System.Data.SqlTypes.SqlBinary.Null); }
            if (Item.TestAfterPicture != null) { objParameter.Add("@TestAfterPicture", Item.TestAfterPicture); }
            else { objParameter.Add("@TestAfterPicture", System.Data.SqlTypes.SqlBinary.Null); }
            ...
            var oldWashData = this.GetMockupWash_Detail(new MockupWash_Detail() { ReportNo = Item.ReportNo }).ToList();
...
";
        ...
        public int DeleteMockupWash(MockupWash Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
            SbSql.Append("SET XACT_ABORT ON" + Environment.NewLine);
            ...
            SbSql.Append(@"DELETE FROM SciPMSFile_MockupWash" + Environment.NewLine);
            SbSql.Append("WHERE ReportNo = @ReportNo" + Environment.NewLine);
            objParameter.Add("@ReportNo", DbType.String, Item.ReportNo);
            return ExecuteNonQuery(CommandType.Text, SbSql.ToString(), objParameter);
        }
...
SELECT {top1}
		...
		,m.HTCoolingTime
        ,Type
        ,TestBeforePicture = (select top 1 TestBeforePicture from SciPMSFile_MockupWash mi WITH(NOLOCK) where m.ReportNo=mi.ReportNo)
        ,TestAfterPicture = (select top 1 TestAfterPicture from SciPMSFile_MockupWash mi WITH(NOLOCK) where m.ReportNo=mi.ReportNo)
        ,AddDate
        ,AddName
        ,EditDate
        ,EditName
        ,Signature = (select t.Signature from Technician t where t.ID = Technician)
        ,[MockupWash_Approver] = m.Approver
        ,[MockupWash_ApproverName] = (select t.Name from Pass1 t where t.ID = m.Approver)
FROM MockupWash m WITH(NOLOCK)
...
");
        ...
        public int CreateDetail(MockupWash_Detail Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
        ...
        public int DeleteDetail(MockupWash_Detail Item)
        {
            StringBuilder SbSql = new StringBuilder();
            SQLParameterCollection objParameter = new SQLParameterCollection();
